...
<?php
    ...
    try {
        ...
        $articles = $bdd->query('SELECT titre,resumee, id FROM article ORDER BY id DESC');
        $articles->bindParam("sss")
        if($txt != "") $articles .= "where titre like '".$txt."%' ";
        $articles .= "LIMIT 3 ";

        //execution et récup des résultats dans $rows
        $req = $bdd->query($articles);
        $rows = $req->fetchAll();
    } catch (Exception $e) {  }
?>
